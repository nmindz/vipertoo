...
wine_build_environment_prechecks() {
	...
	if use abi_x86_64; then
		...
		if (( using_gcc && (gcc_major_version == 5 && gcc_minor_version <= 3) )); then
			...
			ewarn "and are no longer supported by Gentoo's Toolchain Team."
...
src_prepare() {
	...
	# Apply Andre Awesie's Overwatch Patches by default, but allow disabling it for regular Wine Staging version
	if use overwatch; then
		#PATCHES+=( "${FILESDIR}/awesie/0001-Apply-all-Wine-Staging-changes.patch" )
		PATCHES+=( "${FILESDIR}/awesie/0002-Port-some-wine-overwatch-patches.patch" )
		PATCHES+=( "${FILESDIR}/awesie/0003-Partially-implement-BCryptImportKey.patch" )
		PATCHES+=( "${FILESDIR}/awesie/0004-Remove-an-assert-from-wined3d.patch" )
		PATCHES+=( "${FILESDIR}/awesie/0005-Use-awesie-s-new-flat-interpolation-patches.patch" )
		PATCHES+=( "${FILESDIR}/awesie/0006-Handle-block-size-in-get_gnutls_cipher.patch" )
		PATCHES+=( "${FILESDIR}/awesie/0007-Edit-README.patch" )
	fi
